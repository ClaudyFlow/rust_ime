...
### 5. 极速自动上屏 (Auto Commit)
**针对唯一匹配结果的极致提速。**
- **英文辅码自动上屏**：在英文模式下使用大写辅码（如 `liI`），若匹配结果唯一，无需按空格即可自动上屏并追加空格。
- **精准匹配自动上屏**：当输入的拼音或序列在词库中拥有全局唯一的精准匹配结果时，实现零击键自动上屏。
- **配置方式**：在网页配置中心“系统总览”中可独立开启。
...
## 🛠 命令行参数与后台管理 (Command Line Options)

`rust-ime` 默认在后台运行，不会占用你的终端窗口。

| 参数 | 说明 |
| :--- | :--- |
| **(无参数)** | **默认模式**。程序将自动转入后台运行，并将日志输出到 `/tmp/rust-ime.out`。 |
| **`--foreground`** | **前台调试模式**。在当前终端显示所有运行日志，适合开发或排查问题时使用。 |
| **`--install`** | **安装自启**。在 `~/.config/autostart/` 下创建启动项，实现开机自动加载。 |
| **`--stop`** | **停止运行**。一键杀掉所有后台运行的 `rust-ime` 进程。 |

> **提示**：即使在控制台关闭后，后台运行的程序依然会忽略 `SIGHUP` 信号，确保持续可用。
