...
### 1. 硬件级拦截 (Evdev & Uinput)
- **底层驱动**：通过 `evdev` 直接从 `/dev/input/event*` 拦截原始键盘事件，实现 100% 的全局快捷键接管，无视 Wayland 合成器的按键转发限制。
- **虚拟键盘**：通过 `uinput` 创建虚拟键盘设备，将处理后的字符（或粘贴指令）注入内核输入流，具备极高的系统兼容性。
...
- **拼音切分**：采用递归与贪心结合的切分算法，支持 `v` 替代 `ü` 及 `	exttt{` 隔音符号。
- **联想推理**：基于 **N-Gram** 语言模型，结合用户历史输入频率，动态调整候选词权重。
- **差量渲染**：独创的拼音预览差量更新算法，在退格时仅发送必要的删除信号，彻底消除在 Firefox 等重型应用中的闪烁与卡顿。
