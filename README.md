...
1. **环境准备**：
   - 编译项目：`cargo build` (生成主程序) 和 `cargo build --lib` (生成 TSF DLL)。
...
3. **注册与注销 (TSF 输入法)**：
   - **注册**：`.\target\debug\rust-ime.exe --register`
     *注册成功后，在 Windows 设置 -> 语言 -> 输入法中添加 "Rust IME"。*
   - **注销**：`.\target\debug\rust-ime.exe --unregister`

4. **运行服务**：
   - **后台运行**：`.\target\debug\rust-ime.exe` (或使用 `--daemon`)
   - **前台调试**：`.\target\debug\rust-ime.exe --foreground`
   - **停止服务**：`.\target\debug\rust-ime.exe --stop`

### Windows 特有功能
- **原生分层窗口**：基于 Win32 API 实现的高性能透明候选窗。
- **系统托盘图标**：右键托盘图标可进行模式切换、词库选择及各项功能开关。
- **系统通知**：切换模式时通过 Windows 通知中心（Toast）提供反馈。
- **自动开机自启**：`.\target\debug\rust-ime.exe --install` 可写入注册表实现自启。
