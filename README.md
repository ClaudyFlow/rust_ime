...
![界面展示](readmepicture/ni_without_style.png)
![界面展示](readmepicture/niP.png)
![界面展示](readmepicture/gn.png)
![界面展示](readmepicture/gnG.png)
...
### 1. 英文辅助码：解决重码的终极方案
这是 rust-ime 的特色功能。在拼音重码极多时，您无需翻页，只需输入一个**Shift + 大写字母**即可根据“义”精准定位。
...
*   **操作方法**：
    *   输入拼音后，输入 **分号 `;`** 键，随后直接输入 **小写字母** 辅助码。
    *   **示例**：
        *   输入 `ren` ⮕ 候选词：人、任、认...
        *   输入 `ren;v` ⮕ **人** (首两笔为“撇捺”，撇3+捺4 -> `v`)。
        *   输入 `de` ⮕ 候选词：的、得、地...
        *   输入 `de;rp` ⮕ **的** (前两笔“撇竖”=32=`r`，末两笔“点折”=45=`p` -> `rp`)。
*   **并存**：支持 `拼音;笔画Shift英文` 混合过滤（如 `li;mfC`）。
