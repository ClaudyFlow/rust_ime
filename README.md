...
*   **布局逻辑 (5x5 矩阵)**：采用 QWER 布局，将 5 种基本笔画映射到 5x5 的按键矩阵中，每 1 个按键代表 **2 个连续笔画**。
    *   **行**：代表该对笔画中的 **第 1 笔**。
    *   **列**：代表该对笔画中的 **第 2 笔**。
    *   **笔画索引**：1:横(QWER), 2:竖(ASDF), 3:撇(ZXCV), 4:捺/点(YUIO), 5:折(HJKL)。
    *   **示例按键**：`q`=横横, `w`=横竖, `a`=竖横, `s`=竖竖, `z`=撇横, `y`=捺横, `h`=折横。
*   **编码规则**：
...
*   **操作方法**：
    ...
    *   **示例**：
        *   输入 `ren` ⮕ 候选词：人、任、认...
        *   输入 `renv` ⮕ **人** (首两笔为“撇捺”，对应按键 `v`)。
        *   输入 `de` ⮕ 候选词：的、得、地...
        *   输入 `dexy` ⮕ **的** (前两笔“撇竖”=`x`，末两笔“捺横”=`y`)。
