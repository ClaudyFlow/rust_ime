...
### 1. 万能辅助码过滤 (Shift + 字母)：本输入法的核心特色
这是 rust-ime 最具竞争力的功能，专为追求极致速度的盲打极客设计。当拼音输入出现大量重码时，您无需查看候选窗，只需凭直觉按下 **Shift + 目标字的特征字母**，即可瞬间锁定目标。

*   **操作方式**：
    *   先输入拼音（如 `ma`）。
    *   **同时按下** 或 **先按住 Shift 再按** 某个字母（如 `D`）。
    *   系统会立即过滤当前所有候选词，只保留那些辅助码（笔画、含义等）匹配该字母的结果。
*   **示例**：
    *   输入 `ma` ⮕ 候选词：妈、马、嘛、码、玛...
    *   输入 `ma + Shift + D` ⮕ **码** (其笔画辅助码为 `Dx`) 瞬间排到第一或唯一。
    *   输入 `li + Shift + C` ⮕ **礼** (Ceremony) 瞬间排到第一。
*   **核心优势**：
    *   **左右 Shift 通用**：完全符合双手打字习惯。
    *   **粘性连续过滤**：支持连续输入多个辅助码（如 `Shift+D` 之后再按 `X`）进行多级精准定位。
    *   **语义与形状双修**：系统会自动匹配候选词的英文含义（义）或笔画特征（形），无论你习惯哪种思维，都能快速找到目标。
*   **声明**：这是本输入法的灵魂功能，建议在熟悉后完全替代数字键选词，实现真正的全键盘盲打。

### 2. 笔画输入法与辅助码 (SBSRF 双笔一键系统)
**这是本引擎最具特色的高效输入方案，将汉字笔画映射为 5x5 的字母矩阵。**

*   **核心思想**：**一键代表两笔**。摒弃传统的数字键 (1-5) 输入，直接使用键盘上的字母进行极速编码。
*   **布局逻辑 (5x5 矩阵)**：将 5 种基本笔画（1:横, 2:竖, 3:撇, 4:捺/点, 5:折）映射到 QWERTY 键盘。
    *   **矩阵行**：代表该按键对应的 **第 1 笔**。
    *   **矩阵列**：代表该按键对应的 **第 2 笔**。
    *   **完整映射表**：
        | 第1笔 \ 第2笔 | 1:横 | 2:竖 | 3:撇 | 4:捺 | 5:折 |
        | :--- | :---: | :---: | :---: | :---: | :---: |
        | **1:横** | **G** (11) | **F** (12) | **D** (13) | **S** (14) | **A** (15) |
        | **2:竖** | **H** (21) | **J** (22) | **K** (23) | **L** (24) | **M** (25) |
        | **3:撇** | **T** (31) | **R** (32) | **E** (33) | **W** (34) | **Q** (35) |
        | **4:捺** | **Y** (41) | **U** (42) | **I** (43) | **O** (44) | **P** (45) |
        | **5:折** | **N** (51) | **B** (52) | **V** (53) | **C** (54) | **X** (55) |

*   **两种使用场景**：
    1.  **独立笔画输入法**：按下 `` ` `` 键切换至 `stroke` 方案。此时直接输入字母（如 `fth`）即可匹配笔画序列。**注意：此模式下不使用数字 1-5 输入笔画。**
    2.  **拼音笔画辅助码**：在输入拼音后，输入分号 `;`，随后输入的字母将作为笔画过滤条件。
        *   示例：`de;rp` ⮕ **的** (前两笔“撇竖”=32=`r`，末两笔“点折”=45=`p`)。

*   **编码规则**：
    *   **单字 (全码)**：依次取笔画对。适用于所有汉字。
    *   **单字 (定长 4 键简码)**：针对 8 笔以上（全码 > 4 键）的汉字，统一采用 4 键编码。
        *   **规则**：取全码的 **前 3 键 + 最后一个键** (类似五笔)。
    *   **万能通配符 (Z 键)**：当不确定中间笔画时，可用 `z` 键代替。
        *   **功能**：`z` 匹配任意一个编码字母（即 2 笔）。
        *   **示例**：输入 `fzh` 可匹配所有以 `f` 开头、`h` 结尾的三键编码（共 6 笔）。
    *   **词组 (定长 4 键制)**：为了提升输入效率，词组统一采用 4 键编码。
        *   **2字词**：第1字前2键 + 第2字前2键。
            *   示例：`中国` ⮕ `m` (中1) + `n` (中2) + `a` (国1) + `h` (国2) = `mnah`。
        *   **3字词**：第1字首键 + 第2字首键 + 第3字前2键。
            *   示例：`计算机` ⮕ `y` (计1) + `t` (算1) + `f` (机1) + `s` (机2) = `ytfs`。
        *   **4字及以上**：前3字首键 + 最后一个字首键。
            *   示例：`中华人民共和国` ⮕ `m` (中1) + `r` (华1) + `r` (人1) + `a` (国1) = `mrra`。
    *   **优势**：极大减少击键次数，编码密度高，重码率极低，且具备极佳的录入节奏感。
