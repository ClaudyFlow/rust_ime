...
- **后台守护模式**：通过 `--daemon` 参数启动，自动隐藏控制台窗口。
- **稳定上屏逻辑**：适配现代记事本 (Notepad) 等应用的文本注入。

### Windows 安装说明
1. 编译项目：`cargo build`。
2. **以管理员身份**运行：`.\target\debug\rust-ime.exe --register` 以注册 TSF 组件。
3. 在系统设置中添加 **Rust IME** 键盘。
4. 启动服务：`.\target\debug\rust-ime.exe --daemon`。
