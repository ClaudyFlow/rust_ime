...
pub fn start_gui(rx: Receiver<GuiEvent>, initial_config: Config) {
    ...
    unsafe {
        ...
        let hwnd = CreateWindowExW(
            ...
            100, 100, 600, 120, None, None, instance, None,
                                                ...
                                                if state.pinyin.is_empty() {
                                                    ...
                                                    let current_hints = if state.hints.len() >= end {
                                                        ...
                                                        vec![String::new(); current_candidates.len()]
                                                                                    ...
                                                                                    // 计算实际绘制出的图像宽度 (由于我们目前是 Vec<u8>，这里通过像素点数量反推，或者在 draw 里返回)

                    

                                                                                    // 简化处理：目前 draw 内部根据内容决定了 pixmap 大小，我们需要同步窗口大小
                                                                                    ...
                                                                                    SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, dynamic_width as i32, painter.height as i32, SWP_NOMOVE | SWP_NOACTIVATE);
