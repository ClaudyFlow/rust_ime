...
fn compile_dict_for_path(src_dir: &str, out_stem: &str, is_english: bool, syllables: &HashSet<String>, freq_map: &HashMap<String, u64>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (_, candidates) in entries.iter_mut() {
        candidates.sort_by(|(wa, _), (wb, _)| freq_map.get(wb).unwrap_or(&0).cmp(freq_map.get(wa).unwrap_or(&0)));
    }
...
fn process_yaml_file(path: &Path, entries: &mut BTreeMap<String, Vec<(String, String)>>, abbrev_entries: &mut BTreeMap<String, Vec<(String, String)>>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for line in reader.lines() {
        ...
        if parts.len() >= 2 {
            ...
            entries.entry(pinyin).or_default().push((parts[0].to_string(), weight.to_string()));
            let abbrev: String = parts[1].split_whitespace().filter_map(|s| s.chars().next()).collect();
            if abbrev.len() > 1 && abbrev != parts[1].replace(' ', "") {
                abbrev_entries.entry(abbrev).or_default().push((parts[0].to_string(), weight.to_string()));
            }
        }
    }
    Ok(())
                    ...
                    for v in arr {
                        if let Some(s) = v.as_str() { 
                            ...
                            if let Some(c) = o.get("char").and_then(|c| c.as_str()) {
                                let mut hint = o.get("en").and_then(|e| e.as_str()).unwrap_or("").to_string();
                                if let Some(t) = o.get("tone").and_then(|t| t.as_str()) { if !hint.is_empty() { hint = format!("{} {}", t, hint); } else { hint = t.to_string(); } }
                                entries.entry(key.clone()).or_default().push((c.to_string(), hint.clone()));
                                if let Some(ref a) = abbrev { abbrev_entries.entry(a.clone()).or_default().push((c.to_string(), hint)); }
                            }
                        }
                    }
                }
            } else if let Some(s) = val.as_str() {
                if is_english { entries.entry(key.clone()).or_default().push((key.clone(), s.to_string())); }
                else {
                    entries.entry(key.clone()).or_default().push((s.to_string(), String::new()));
                    if let Some(ref a) = abbrev { abbrev_entries.entry(a.clone()).or_default().push((s.to_string(), String::new())); }
                }
...
fn write_binary_dict(idx_path: &str, abbrev_idx_path: &str, dat_path: &str, entries: BTreeMap<String, Vec<(String, String)>>, abbrev_entries: BTreeMap<String, Vec<(String, String)>>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (k, mut pairs) in entries {
        let mut seen = HashSet::new(); pairs.retain(|(c, _)| seen.insert(c.clone()));
...
fn encode_block(pairs: &[(String, String)]) -> Vec<u8> {
    ...
    for (w, h) in pairs {
        let wb = w.as_bytes(); let hb = h.as_bytes();
        b.extend_from_slice(&(wb.len() as u16).to_le_bytes()); b.extend_from_slice(wb);
        b.extend_from_slice(&(hb.len() as u16).to_le_bytes()); b.extend_from_slice(hb);
    }
    b
}
